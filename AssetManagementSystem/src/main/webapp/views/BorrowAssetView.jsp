			...
			<div class="col-sm-12 general-setting shadow-sm p-3 mb-5 bg-gray">
					...
					<div class="title-feature">
						<div class="text-right">
							<input type="text" value="${asset.getId()}"
								style="display: none;" name="rfid_code"> <input
								style="display: none;" name="borrow_cd" value="${asset.getId()}">
							...
							<c:if test="${asset.getStatus()=='1'}">
								...
								data-target="#ModalApprove"  style="border-radius: 0" name="back"
									...
									<i class="fa fa-arrow-circle-right" style="font-size: 24px"></i>CHƯA
									PHÊ DUYỆT
								</button>
							</c:if>
							</c:if>
							...
							<c:if test="${asset.getStatus()=='4'}">
								<button type="button" data-toggle="modal"
								data-target="#ModalConfirm"  style="border-radius: 0" name="back"
									class="btn btn-primary">
									<i class="fa fa-arrow-circle-right" style="font-size: 24px"></i>XÁC
									NHẬN TRẢ
								</button>
							</c:if>
							</c:if>
				...
				<c:if test="${asset!=null}">
					<div class="row">
							...
							<div class="form-group">
								...
								<div class="input-group">
									<input class="form-control" readonly="readonly"
										value="${asset.getAsset().getModel()}" />
								</div>
							</div>
						</div>
						<div class="col-sm-4">
							<div class="form-group">
								<label class="title_input">SỐ SERIES:</label> <input
									class="form-control" readonly="readonly"
									value="${asset.getAsset().getSeries()}" />
							</div>
						</div>
						<div class="col-sm-4">
							<div class="form-group">
								<label class="title_input">DOANH NGHIỆP CHO MƯỢN:</label> <input
									class="form-control" readonly="readonly"
									value="${asset.getCmpn_master().getCompany_shortname()}" />
							</div>
						</div>
						<div class="col-sm-4">
							<div class="form-group">
						...
						<div class="col-sm-4">
							<div class="form-group">
								<label class="title_input">NGÀY CHO MƯỢN:</label> <input
									class="form-control" readonly="readonly"
									value="${asset.getDate_start()}" />
							</div>
						</div>
						<div class="col-sm-4">
							<div class="form-group">
								<label class="title_input">DOANH NGHIỆP MƯỢN:</label> <input
									class="form-control" readonly="readonly"
									value="${asset.getCmpn_client().getCompany_shortname()}" />
							</div>
						</div>
						...
						<div class="col-sm-4">
							<div class="form-group">
								<label class="title_input">NGÀY TRẢ:</label> <input
									class="form-control" readonly="readonly"
									value="${asset.getDate_end()}" />
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group">
								...
								<textarea class="form-control" style="resize: none;"
									readonly="readonly">${asset.getReason()}</textarea>
							</div>
						</div>
					...
					<form action="BorrowAssetApprove" method="post"
												...
												<input style="display: none;" name="borrow_cd"
													value="${asset.getId()}">
												<button
													style="width: 160px; margin: auto; height:40px; background-color: orange"
													type="submit" class="view" name="view">XEM LỆNH</button>
					</form>
				</c:if>
			</div>
		</div>
		...
		<div class="modal-dialog modal-xl modal-dialog-centered"
			...
			<div class="modal-content iset3d"
				...
				<div class="modal-header"
					...
					<p class="modal-title"
						style="height: 30px; margin-top: -9px; color: white; font-weight: 700">PHÊ DUYỆT CHO MƯỢN</p>
				</div>
				...
				<div class="modal-body">
					...
					<input
							...
							<c:if test="${asset.getStatus()=='1'}">
								<label for="email"
									style="font-weight: 700; text-transform: uppercase;">PHƯƠNG THỨC PHÊ DUYỆT:</label>
								<br>
								<label style="margin-right: 30px"><input type="radio"
									...
									style="color: green; font-weight: 700">PHÊ DUYỆT</span></label>
								<label> <input type="radio" class="form-control "
									name="accept_radio" id="disApprove" value="6"> <span
									style="color: red; font-weight: 700">KHÔNG PHÊ DUYỆT</span>
								</label>
							</c:if>
							<c:if test="${asset.getStatus()!='1'}">
								<label for="email"
						...
						<div class="form-group" style="margin-bottom: 0px;">
							<label for="pwd" style="font-weight: 700;">LÝ DO KHÔNG PHÊ DUYỆT:</label><br>
						...
						<button type="submit" onsubmit="return checkValud()"
							name="approve" style="margin-top: 10px;"
							class="btn btn-danger">LƯU PHÊ DUYỆT</button>
			...
			<div class="modal-content iset3d"
				...
				<div class="modal-body">
					<form action="BorrowAssetConfirmReturn" method="POST">
							...
							<br>
						<input style="display: none;" name="borrow_cd" value="${asset.getId()}">

						
						<button type="submit" onsubmit="return checkValud()"
							name="confirm" style="margin-top: 10px;"
							class="btn btn-danger">XÁC NHẬN CÓ TRẢ</button>
							    <button type="button" style="margin-top: 10px;"
							class="btn btn-danger" data-dismiss="modal">ĐÓNG</button>
					</form>
				</div>

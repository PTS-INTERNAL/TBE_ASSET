			...
			<div
				...
				<form action="BorrowAssetApprove" method="POST">
					<input type="text" class="form-control"
						style="height: 40px; display: none;" name=borrow_cd_exp
						readonly="readonly" value="${borrow_cd_exp}" id="cmpn_na_client">
					<div class="title-feature">
						<div class="text-right">
							<button type="submit" name="print" class="btn btn-primary">
								<i class="fa fa-arrow-circle-right" style="font-size: 24px"></i>XUẤT
								LỆNH ĐIỀU ĐỘNG
							</button>
				...
				<div class="row">
					...
					<div class="col-sm-4">
						<div class="form-group">
						<label class="title_input">MÃ LỆNH XUẤT: </label>
							<select name="codeExport" class="form-control">
						...
						<c:forEach var="p" items="${lstBr}">
							<tr>
								...
								<td><c:if test="${p.getStatus()=='1'}">
										<span class="view"
											style="width: 160px; background-color: orange; color: black; border: 1px solid yellow;">CHƯA
											DUYỆT</span>
										</form>
										...
										<span class="view"
											style="border: 1px solid white; width: 160px; background-color: red; color: black">XÁC
											NHẬN TRẢ</span>
								...
								<td style="width: 120px; text-align: center">
									<form action="BorrowAssetApprove" method="POST">
										<input style="display: none;" name="borrow_cd"
											value="${p.getId()}">
										<button type="submit" class="view" style="width: 100px;"
								...
								<td style="width: 170px; text-align: center">
									<form action="BorrowAssetApprove" method="POST">
										<input style="display: none;" name="borrow_cd"
											value="${p.getId()}"> <input type="text"
											class="form-control" name="reason" id="reasonhide<%=stt%>"
											value="${p.getReason_not_allow()}"
											style="display: none; height: 30px; margin: 2px;">
										<button type="submit" class="view" style="width: 150px;"
											name="disApprove">KHÔNG DUYỆT</button>
									</form>
								</td>
								<td><input type="text" class="form-control" name="abc"
									id="reason<%=stt%>" onchange="setReason(<%=stt%>)"
									value="${p.getReason_not_allow()}"

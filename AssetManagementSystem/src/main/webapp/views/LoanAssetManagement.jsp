...
<head>
<meta charset="UTF-8">
<title>QUẢN LÝ MƯỢN TÀI SẢN</title>
	...
	<div style="padding: 0px; ">
		<div class="row">
			<div class="col-sm-12 general-setting shadow-sm p-3 mb-5 bg-gray">
				<form action="LoanAssetManagement" method="POST">
							...
							<tr>
								...
								<td><div class="input-group">
										<span class="input-group-addon"> </span> <input type="text"
											...
											value="${loan.getDept_master().getDept_name() }" readonly="readonly" name="department_name_master"
											id="department_name_master"> <input type="text"
											class="form-control inputAssetItem" style="display: none;"
											name="department_cd_master" value="${loan.getDept_master().getDept_cd() }"
								...
								<td><input class="selectList" value="${loan.getAsset().getName()}"
									name="text_asset_name"></td>
								<td class="title">Model</td>
								<td><input value="${loan.getAsset().getModel()}"  name="text_model">
								</td>
								<td class="title">Số Series</td>
								<td><input value="${loan.getAsset().getSeries()}"  name="text_series">
								</td>
							</tr>
							<tr>
								...
								<td><input class="selectList"
									value="${loan.getAsset().getRFID()}" name="text_rfid"></td>

								<td class="title">Ngày mượn</td>
								<td colspan="3"><input value="${borrow.getDate_start()}"
									...
									value="${loan.getDate_start_end()}" style="width: 45%"
				...
				<c:if test="${lst.size() > 0}">
					<table id="table.data" style="margin-top: 0px"
						...
						<thead>
							<tr>
								<th colspan="1"></th>
								<th colspan="3">DOANH NGHIỆP MƯỢN</th>
								<th colspan="4">THÔNG TIN TÀI SẢN</th>
								<th colspan="5">DOANH NGHIỆP CHO MƯỢN</th>
						...
						<tbody>
							...
							<c:forEach var="p" items="${lst}">
								<tr>
									...
									<td>${p.getCmpn_master().getCompany_shortname()}</a></td>
									...
									<c:if test="${p.getStatus()=='1'}">
										<form action="LoanAssetConfirm" method="post" style="width:160px;">
										<input style="display:none;" name="loan_cd" value="${p.getId()}">
										<button style="width:160px; margin:auto;background-color: orange" type="submit" class="view" name="view">CHƯA XÁC NHẬN</button>
										</form>
									</c:if>
									<c:if test="${p.getStatus()=='2'}">
										<form action="LoanAssetReturn" method="post" style="width:160px;">
										<input style="display:none;" name="loan_cd" value="${p.getId()}">
										<button style="width:160px; background-color:yellow; color: black;border: 1px solid yellow;" type="submit" class="view" name="view">ĐANG MƯỢN</button>
										</form>
									...
									<c:if test="${p.getStatus()=='3'}">
										...
										<form action="LoanAssetApprove" method="post" style="width:160px;">
										<input style="display:none;" name="loan_cd" value="${p.getId()}">
										<button type="submit" class="view" style="width:160px;  margin:auto; background-color: red;" name="view_asset">CHƯA DUYỆT TRẢ</button>
										</form>
									</c:if>
									<c:if test="${p.getStatus()=='4'}">
										<span class="view" style="border: 1px solid white; width:160px; background-color:green; color: white">CHỜ XÁC NHẬN</span>
									</c:if>
									<c:if test="${p.getStatus()=='5'}">
										<span class="view" style="width:160px; background-color:white; color: black; border:0px;">ĐÃ TRẢ</span>
									</c:if>
									<c:if test="${p.getStatus()=='6'}">
										<span class="view" style="width:160px; background-color:red; color: white; border:0px;">KHÔNG MƯỢN</span>
									</c:if>
									
									</td>
									<td><form action="LoanAssetView" method="post" style="width:60px;">
										<input style="display:none;" name="id_borrow" value="${p.getId()}">

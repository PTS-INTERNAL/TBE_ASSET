...
import com.ams.dao.loan.LoanCouponUpdateApproveAccountantDao;
import com.ams.dao.loan.LoanCouponUpdateDisApproveAccountantDao;
...
@Controller
@RequestMapping("/LoanCouponManagmentAccountApprove")
public class LoanCouponManagmentAccountantApprove {
	String TITLE = "MÀN HÌNH DUYỆT PHIẾU MƯỢN TÀI SẢN SẢN CỦA KẾ TOÁN";
	...
	public ModelAndView init(HttpServletRequest request)
	{
		...
		mv.setViewName("views/LoanCouponManagmentAccountantApprove.jsp");
		return mv;
	}
	...
	public ModelAndView approve(HttpServletRequest request)
	{
		...
		if(Common.isEmpty(status_device))
		{
			...
			LoanCouponUpdateApproveAccountantDao updateApprove = new LoanCouponUpdateApproveAccountantDao(brCoupn);
			try {
				updateApprove.excute();
				Common.showMessageNotification(mv, "PHÊ DUYỆT THÀNH CÔNG");
				
				//Cập nhật phiếu cho mượn
				LoanCouponModel itemModel = new LoanCouponModel();
				itemModel.setCoupon_cd(coupon_cd);
				
				LoanCouponSelectDao selectDao = new LoanCouponSelectDao(itemModel);
				List<LoanCouponModel> lstCoupon = selectDao.excute();
				if(lstCoupon.size()==1)
				{
					String loan_borrow_cd = lstCoupon.get(0).getCoupon_borrow_cd();
					if(Common.isNotEmpty(loan_borrow_cd))
					{
						BorrowCouponModel borrowItem = new BorrowCouponModel();
						borrowItem.setStatus("5");
						borrowItem.setCoupon_cd(loan_borrow_cd);
						
						BorrowCouponUpdateStatusDao updateStatus = new BorrowCouponUpdateStatusDao(borrowItem);
						updateStatus.excute();
					}
				}
	...
	public ModelAndView disApprove(HttpServletRequest request)
	{
		...
		if(Common.isEmpty(reason_not_allow))
		{
			...
			SystemControl sys = new SystemControl(request);
			brCoupn.setUpdate_user(sys.EmployeeCD);
			LoanCouponUpdateDisApproveAccountantDao updateApprove = new LoanCouponUpdateDisApproveAccountantDao(brCoupn);
			try {
				updateApprove.excute();

...
@Controller
@RequestMapping("/LoanCouponManagement")
public class LoanCouponManagementController {
	String TITLE = "MÀN HÌNH QUẢN LÝ PHIẾU MƯỢN TÀI SẢN SẢN";
	
	@RequestMapping(params="returnAsset", method = RequestMethod.POST)
	public String returAsset(HttpServletRequest request)
	{
		return "rediect:/LoanCouponAssetReturn";
	}
	...
	public ModelAndView init(HttpServletRequest request)
	{
		...
		LoanCouponModel br  = new LoanCouponModel();
		br.getCmpn_master().setCompany_cd((String) request.getSession().getAttribute(SessionConstants.SUB_SYSTEM_CD));
		LoanCouponSelectDao BASD = new LoanCouponSelectDao(br);
		
		try {
			List<LoanCouponModel> lst = BASD.excute();
			if(lst.size()==0)
			{
			...
			{
				mv.addObject(ParamsConstants.MESSAGE_NOTIFICATION, "TÌM THẤY "+lst.size()+ " PHIẾU MƯỢN ĐƯỢC TÌM THẤY");
		...
		mv.setViewName("views/LoanCouponManagement.jsp");
		return mv;
	}
	...
	public ModelAndView Search(HttpServletRequest request)
	{
		...
		LoanCouponModel coupon = new LoanCouponModel();
		...
		LoanCouponSelectDao BASD = new LoanCouponSelectDao(coupon);

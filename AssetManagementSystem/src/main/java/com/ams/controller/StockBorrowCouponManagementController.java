...
@Controller
@RequestMapping("/StockBorrowCouponManagement")
public class StockBorrowCouponManagementController {
	String TITLE = "MÀN HÌNH QUẢN LÝ PHIẾU CHO MƯỢN TÀI SẢN SẢN CỦA KHO VẬN";
	...
	public ModelAndView init(HttpServletRequest request)
	{
		...
		br.setStatus("7777");
		br.setStockApprrove(true);
		
		mv.addObject("coupon", br);
		...
		LoanCouponModel brs  = new LoanCouponModel();
		brs.getCmpn_master().setCompany_cd((String) request.getSession().getAttribute(SessionConstants.SUB_SYSTEM_CD));
		brs.setStatus("5");
		LoanCouponSelectDao BASDs = new LoanCouponSelectDao(brs);
		
		try {
			List<LoanCouponModel> lsts = BASDs.excute();
			if(lsts.size()==0)
			{
				//mv.addObject(ParamsConstants.MESSAGE_ERROR, "KHÔNG CÓ THẤY PHIẾU MƯỢN NÀO ĐƯỢC TÌM THẤY");
			}
			else
			{
				mv.addObject(ParamsConstants.MESSAGE_NOTIFICATION, "TÌM THẤY "+lsts.size()+ " PHIẾU MƯỢN ĐƯỢC TÌM THẤY");
				for(int i =0;i<lsts.size();i++)
				{
					if(Common.isNotEmpty(lsts.get(i).getDate_start()))
					{
						try {
						String dateEndConvert = Common.ConvertStringToDateStr(lsts.get(i).getDate_start(), "yyyy-mm-dd","dd/mm/yyyy");
						lsts.get(i).setDate_start(dateEndConvert);
						} catch (ParseException e1) {
							mv.addObject(ParamsConstants.MESSAGE_ERROR, "DỮ LIỆU TỒN TẠI ĐỊNH DẠNG NGÀY KHÔNG HỢP LỆ");
						}
					}
					if(Common.isNotEmpty(lsts.get(i).getDate_end_schedule()))
					{
						try {
						String dateEndConvert = Common.ConvertStringToDateStr(lsts.get(i).getDate_end_schedule(), "yyyy-mm-dd","dd/mm/yyyy");
						lsts.get(i).setDate_end_schedule(dateEndConvert);
						} catch (ParseException e1) {
							mv.addObject(ParamsConstants.MESSAGE_ERROR, "DỮ LIỆU TỒN TẠI ĐỊNH DẠNG NGÀY KHÔNG HỢP LỆ");
						}
					}
					if(Common.isNotEmpty(lsts.get(i).getDate_end_real()))
					{
						try {
						String dateEndConvert = Common.ConvertStringToDateStr(lsts.get(i).getDate_end_real(), "yyyy-mm-dd","dd/mm/yyyy");
						lsts.get(i).setDate_end_real(dateEndConvert);
						} catch (ParseException e1) {
							mv.addObject(ParamsConstants.MESSAGE_ERROR, "DỮ LIỆU TỒN TẠI ĐỊNH DẠNG NGÀY KHÔNG HỢP LỆ");
						}
					}
				}
				mv.addObject("lsts", lsts);
			}
		} catch (SQLException e) {
			e.printStackTrace();
			mv.addObject(ParamsConstants.MESSAGE_ERROR, "LỖI TÌM DỮ LIỆU : " + e.toString());
		...
		mv.setViewName("views/StockBorrowCouponManagement.jsp");
		return mv;
	}
	...
	{
		return "redirect:/AccountantBorrowCouponRegister";
	}

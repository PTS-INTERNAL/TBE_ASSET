...
import com.ams.dao.loan.LoanCouponUpdateApproveReturnStockDao;
...
@Controller
@RequestMapping("/LoanCouponReturnStockApprove")
public class LoanCouponReturnStockApprove {
	String TITLE = "MÀN HÌNH DUYỆT LỆNH TRẢ MƯỢN TÀI SẢN SẢN CỦA KHO VẬN";
	...
	public ModelAndView init(HttpServletRequest request)
	{
		...
		mv.setViewName("views/LoanCouponReturnStockApprove.jsp");
		return mv;
	}
	...
	public ModelAndView approve(HttpServletRequest request)
	{
		...
		if(isError == 0) {
			...
			LoanCouponUpdateApproveReturnStockDao updateApprove = new LoanCouponUpdateApproveReturnStockDao(brCoupn);
			try {
		...
		try {
			...
			if (lst.size() == 1) {
				...
				BorrowCouponModel brs = new BorrowCouponModel();
				brs.setCoupon_cd(lst.get(0).getCoupon_borrow_cd());
				brs.setStatus("6");
				
				BorrowCouponUpdateStatusDao update = new BorrowCouponUpdateStatusDao(brs);
				update.excute();
